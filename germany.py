...
class Germany():
    ...
    def fill_fields(self, family, date, time, code, soup):
        ...
        if self.category == '375':
            ...
            text_fields = {'2005': family[0]["vc_birth"], '854': family[0]["vc_passport"], '856': family[0]["vc_phone"].replace("+", "").replace(" ", ""), '860': additional_users, '858': f'{len(family)}', '2007': inviting if inviting  else "hotel",'855': 'Geschäft' if family[0]["vc_type"] == 'Buisness' else 'Посещение родственников/друзей/знакомых'}
